...
try {
	...
	$countSql = "SELECT COUNT(*) AS c FROM book $whereSql";
	...
	$listSql = "SELECT id, title, isbn, author, publisher, publication_date, copies, status FROM book $whereSql ORDER BY publication_date DESC, title ASC LIMIT :limit OFFSET :offset";
							...
							<tr>
								...
								<td>
									<?php if (intval($row['status']) !== 1): ?>
										<a href="reservation.php?isbn=<?php echo urlencode($row['isbn']); ?>" class="reserve-button">予約</a>
									<?php else: ?>
