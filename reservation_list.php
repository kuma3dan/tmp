...
try {
	...
	$countSql = "SELECT COUNT(*) FROM reservation WHERE user_id = :user_id AND reservariton_status = 0";
			...
			<?php if ($totalCount > 0): ?>
				...
				<?php
					...
					if ($page > 1) {
						...
						echo '<a class="page-link" href="reservation_list.php?' . htmlspecialchars(http_build_query($queryBase), ENT_QUOTES, 'UTF-8') . '">前へ</a>';
					}
					echo '<span class="page-info">' . $page . ' / ' . $totalPages . '</span>';
					if ($page < $totalPages) {
						$queryBase['page'] = $page + 1;
						echo '<a class="page-link" href="reservation_list.php?' . htmlspecialchars(http_build_query($queryBase), ENT_QUOTES, 'UTF-8') . '">次へ</a>';
					}
					echo '</div>';
				?>
			<?php else: ?>
